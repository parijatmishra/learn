 ...
 *
 */
public class MSEntityRepository
{
    private static final Logger LOG = LoggerFactory.getLogger("db.MSEntity");

    private final IDGenerator idGenerator;
    private final LinkRegistry linkRegistry;

    @Inject
    MSEntityRepository(final IDGenerator idGenerator, final LinkRegistry linkRegistry)
    {
        this.idGenerator = idGenerator;
        this.linkRegistry = linkRegistry;
    }

    Page<MSEntity> list(final Transaction _transaction, final String _startId, final int _count)
    {
        ...
        StatementResult _result = _transaction.run(
                "MATCH (node:MSEntity) WHERE node._id >= $startId RETURN node LIMIT $count",
                _params);
        final List<MSEntity> _dtos = _result.list(rec -> MSEntity.dto(rec.get("node")));
        final String _nextStartId = _dtos.size() > _count ? _dtos.remove(_count)._id : null;
        final Page<MSEntity> _ret = new Page<>(_startId, _count, _dtos, _nextStartId);

        LOG.debug("list: {}", _ret);
        return _ret;
    }

    MSEntity get(final Transaction _transaction, final String _id)
    {
        ...
        final StatementResult _result = _transaction.run("MATCH (node:MSEntity {_id:$_id}) RETURN node", _params);

        try {
            final Record _record = _result.single(); // consumes the result completely
            final Value _node = _record.get("node");
            return MSEntity.dto(_node);
        } catch (NoSuchRecordException e) {
            throw new Api404MissingResourceException(this.linkRegistry.getResourcePath(MSEntity.class), _id);
        }
    }

    String createNode(final Transaction _transaction, final MSEntity _dto)
    {
        // generated ID
        final String _id = this.idGenerator.getNextId("MSEntity");
        ...
        final StatementResult _result = _transaction.run("CREATE (node:MSEntity $params)",
    ...
    boolean exists(final Transaction _transaction, final String _id)
    {
        ...
        final StatementResult _result = _transaction.run(
                "MATCH (node:MSEntity) WHERE node._id=$_id RETURN node._id AS id LIMIT 1",
                Collections.singletonMap("_id", _id));
        return _result.list().size() == 1;
    }

    void updateNode(final Transaction _transaction, final String _id, final MSEntity _dto)
    {
        ...
        final String UPDATE_NODE_QUERY = Neo4JHelper.getUpdateQuery("MSEntity",
                                                                    _params.keySet(),
                                                                    _propertiesToDelete);
    ...
    boolean deleteNode(final Transaction _transaction, final String _id)
    {
        ...
        final Map<String, Object> _params = Collections.singletonMap("_id", _id);
        try {
            final StatementResult _result = _transaction.run("MATCH (node:MSEntity {_id:$_id}) DELETE node", _params);
            final ResultSummary _summary = _result.summary();
            ...
            if (_e.code().equals("Neo.ClientError.Schema.ConstraintValidationFailed")) {
                throw new Api409ResourceInUseException(this.linkRegistry.getResourcePath(MSEntity.class), _id);
            } else {
                throw _e;
            }
        }
