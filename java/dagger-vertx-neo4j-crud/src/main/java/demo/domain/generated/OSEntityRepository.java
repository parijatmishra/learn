 ...
 *
 */
public class OSEntityRepository
{
    private static final Logger LOG = LoggerFactory.getLogger("db.OSEntity");

    private final IDGenerator idGenerator;
    private final LinkRegistry linkRegistry;

    @Inject
    OSEntityRepository(final IDGenerator idGenerator, final LinkRegistry linkRegistry)
    {
        this.idGenerator = idGenerator;
        this.linkRegistry = linkRegistry;
    }

    Page<OSEntity> list(final Transaction _transaction, final String _startId, final int _count)
    {
        ...
        final StatementResult _list = _transaction.run(
                "MATCH (node:OSEntity) WHERE node._id >= $startId RETURN node LIMIT $count",
                _params);
        final List<OSEntity> _dtos = _list.list(rec -> OSEntity.dto(rec.get("node")));
        final String _nextStartId = _dtos.size() > _count ? _dtos.remove(_count)._id : null;
        final Page<OSEntity> _ret = new Page<>(_startId, _count, _dtos, _nextStartId);

        LOG.debug("list: {}", _ret);
        return _ret;
    }

    OSEntity get(final Transaction _transaction, final String _id)
    {
        LOG.debug("get: _id={}", _id);
        // Parameters to query
        final Map<String, Object> _params = Collections.singletonMap("_id", _id);

        final StatementResult _result = _transaction.run("MATCH (node:OSEntity {_id:$_id}) RETURN node", _params);

        try {
            final Record _record = _result.single(); // consumes the result completely
            final Value _node = _record.get("node");
            return OSEntity.dto(_node);
        } catch (NoSuchRecordException e) {
            throw new Api404MissingResourceException(this.linkRegistry.getResourcePath(OSEntity.class), _id);
        }
    }

    String createNode(final Transaction _transaction, final OSEntity _dto)
    {
        // generated ID
        final String _id = this.idGenerator.getNextId("OSEntity");
        ...
        final StatementResult _result = _transaction.run("CREATE (node:OSEntity $params)",
    ...
    boolean exists(final Transaction _transaction, final String _id)
    {
        ...
        final StatementResult _result = _transaction.run(
                "MATCH (node:OSEntity) WHERE node._id=$_id RETURN node._id AS id LIMIT 1",
                Collections.singletonMap("_id", _id));
        return _result.list().size() == 1;
    }

    void updateNode(final Transaction _transaction, final String _id, final OSEntity _dto)
    {
        ...
        final String UPDATE_NODE_QUERY = Neo4JHelper.getUpdateQuery("OSEntity", _params.keySet(), _propertiesToDelete);
    ...
    boolean deleteNode(final Transaction _transaction, final String _id)
    {
        ...
        final Map<String, Object> _params = Collections.singletonMap("_id", _id);
        try {
            final StatementResult _result = _transaction.run("MATCH (node:OSEntity {_id:$_id}) DELETE node", _params);
            final ResultSummary _summary = _result.summary();
