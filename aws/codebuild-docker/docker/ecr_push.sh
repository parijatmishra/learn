...
if [ ${DOCKER_MAJOR_VERSION} -ge 13 ]; then
    # Create and execute CLI for docker version 13.x and higher -- which does not like '-e' option
    $(aws ecr get-login --no-include-email --region ${AWS_DEFAULT_REGION})
else
    # Create and execute CLI for docker version 1.12.x -- which require '-e' option to work properly
